...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '5678d63f-0a48-4f9d-868f-881be5414ed7'; // ID de Shey_andra nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Shey_andra nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '56411994';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'sheylaalejandraanderson577@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Shey_andra nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'shey-andra-nails';
};
...
console.log('✅ config-negocio.js listo para Shey_andra nails');
